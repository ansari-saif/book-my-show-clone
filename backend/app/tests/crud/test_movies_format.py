...
def test_update_movie_format(db: Session) -> None:
    movie_format = create_random_movie_format(db)
    new_format = "3D"
    movie_format_update = MovieFormatUpdate(format=new_format)
    updated_format = crud.update_movie_format(session=db, db_format=movie_format, format_in=movie_format_update)
